...
class TestResolveObjectColumns:
    def test_integers(self, duckdb_cursor):
        ...
        df_expected_res = pd.DataFrame({"0": pd.Series(data=data, dtype="int32")})
    ...
    def test_analyzing_nulls(self, duckdb_cursor, fill, sample_size, get_data):
        data = get_data(fill)
    ...
    def test_structs_in_nested_types(self, duckdb_cursor):
        ...
        for pair in pairs.values():
            check_struct_upgrade("MAP(VARCHAR, INTEGER)[]", construct_list, pair, duckdb_cursor)

        for key, pair in pairs.items():
            expected_type = "MAP(VARCHAR, MAP(VARCHAR, INTEGER))" if key == "v4" else "STRUCT(v1 MAP(VARCHAR, INTEGER))"
            check_struct_upgrade(expected_type, construct_struct, pair, duckdb_cursor)
    ...
    def test_numpy_object_with_stride(self, duckdb_cursor):
        # Create 2D array in C-order (row-major)
        data = np.zeros((10, 3), dtype=np.int64)
        data[:, 0] = np.arange(10)
        data[:, 1] = np.arange(0, 20, 2)

        df = pd.DataFrame(data, columns=["idx", "evens", "zeros"])
    ...
    def test_integer_conversion_fail(self, duckdb_cursor):
        ...
        # default: VARCHAR
        assert is_string_dtype(converted_col["0"].dtype)
        ...
        x = pd.DataFrame(
            {
                ...
                "2": ConvertStringToDecimal(
                    ["-234234234234.0", "324234234.00000005", -128, 345345, "1E5", "1324234359"]
                ),
            }
        )
